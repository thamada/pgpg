...
このような背景から、**GRAPE**（GRAvity piPE）プロジェクト（[Sugimoto et al. 1990](https://www.nature.com/articles/345033a0)；*Nature* **345**, 33–35, 1990）が始まった。重力相互作用の計算は比較的単純（1 ペアあたり約 20〜30 演算相当、低精度型では LNS を用いるため「浮動小数点」と表記される場合もある）であり、全粒子間の相互作用という並列性が高いため、専用パイプラインハードウェアによる加速が有望だった。
...
| **数値表現** | 減算 20 bit 固定小数点、除算・平方根 14 bit LNS（7 bit 指数・5 bit 仮数）、累積 56 bit 固定小数点 |
...
| 研究 | 内容 |
|------|------|
| [Sugimoto et al. (1990)](https://www.nature.com/articles/345033a0) | GRAPE プロジェクト開始。*Nature* 345, 33–35 (1990)。パイプライン 120 Mflops、並列化で Tflops 級が可能と報告 |
...
| **MN-Core** | Preferred Networks（日本） | 低精度テンソル演算 | 開発者：牧野淳一郎（GRAPE 開発者・PGPG 共著者）。Green500 世界 1 位実績、生成 AI 向け L1000 開発中 |
...
GRAPE は**重力 N 体問題**というドメインに特化し、LNS（Logarithmic Number System）や固定小数点を用いて演算コストを削減した。低精度 AI 専用チップは**深層学習**というドメインに特化し、行列積と活性化の演算パターンに合わせて低精度ユニットを設計する。どちらも、
...
| **日本** | Preferred Networks（MN-Core） | 開発者：牧野淳一郎（GRAPE 開発者）。スパコン向け実績、生成 AI 向け L1000 開発（2026 年目標） |
...
2020 年代の AI チップ開発競争において、日本は **Preferred Networks（PFN）** の MN-Core シリーズが一定の存在感を示している。重要な点として、**MN-Core の開発者は GRAPE 開発者の牧野淳一郎**である。牧野は PGPG の共著者であり、GRAPE-4（1996 年ゴードン・ベル賞）、GRAPE-6（2003 年同特別賞）の開発を主導した。つまり、**専用計算機（GRAPE）→ FPGA アクセラレータ（PROGRAPE/PGPG）→ AI 専用チップ（MN-Core）** という技術の系譜が、同一の研究者によって継承されている。第 1 世代 MN-Core は 2020〜2021 年に Green500 で世界 1 位を 3 度獲得し、第 2 世代 MN-Core 2 は 2023 年稼働開始、2024 年 9 月から販売を開始した。2024 年 8 月には SBI ホールディングスと次世代 AI 半導体開発に向けた資本業務提携（最大 100 億円規模の出資予定）を締結、同年 11 月には生成 AI 推論専用プロセッサ **MN-Core L1000** の開発を発表している。L1000 は三次元積層 DRAM 技術を採用し、既存 GPU と比較して最大 10 倍の高速処理を目指し、2026 年の提供を目標としている。一方で、NVIDIA、AMD、Google、Amazon などに比べて市場規模・エコシステム・顧客基盤の面で存在感は限定的である。
...
- **Sugimoto, D., Chikada, Y., Makino, J., Ito, T., Ebisuzaki, T., & Umemura, M. (1990). A special-purpose computer for gravitational many-body problems. *Nature* 345, 33–35.** [Nature](https://www.nature.com/articles/345033a0) — GRAPE プロジェクト開始。パイプライン 120 Mflops、並列化で Tflops 級が可能と報告
...
- [GRAPE プロジェクト開始（Sugimoto et al. 1990, Nature 345, 33–35）](https://www.nature.com/articles/345033a0)
- [PASJ 42(5), 717 (1990)](https://academic.oup.com/pasj/article/42/5/717) — ※同論文では「浮動小数点」＝ LNS の意で用いられている
