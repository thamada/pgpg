/* --- function list -----------------------------
       void std::generate_pg_log_shift(...);
   -----------------------------------------------*/ 
#include<iostream>
#include<cstdio>
#include<cstring>
...
namespace std{
  ...
  void generate_pg_log_shift(char sdata[][STRLEN],vhdl_description *vd)
  {
    int j;
    int nbit,nshift;
    ...
    if(iflag==0){
      ...
      sprintf((*vd).sde[ie], "                                                           \n" );ie++;
      if(nshift == 2) sprintf((*vd).sde[ie], "  y <= x(PG_WIDTH-1) & x(PG_WIDTH-4 downto 0) & \"00\"; \n" );ie++;
      if(nshift == 1) sprintf((*vd).sde[ie], "  y <= x(PG_WIDTH-1) & x(PG_WIDTH-3 downto 0) & '0';     \n" );ie++;
      if(nshift == 0) sprintf((*vd).sde[ie], "  y <= x(PG_WIDTH-1 downto 0);                          \n" );ie++;
      if(nshift == -1) sprintf((*vd).sde[ie], "  y <= x(PG_WIDTH-1) & '0' & x(PG_WIDTH-2 downto 1);   \n" );ie++;
      if(nshift == -2) sprintf((*vd).sde[ie], "  y <= x(PG_WIDTH-1) & \"00\" & x(PG_WIDTH-2 downto 2);\n" );ie++;
      sprintf((*vd).sde[ie], "                                                           \n" );ie++;
