...
void create_table_conv_ftol(nbit_in,nbit_out,nadd,tablecnt)
int nbit_in,nbit_out,*nadd,tablecnt;
{
  int i,iy,nentry,outmask;
  double inscale,outscale,log();
  FILE *fopen(),*fp;
  char filename[100];
...
{
        int i,ii;
	int nbit_fix,nbit_log,nbit_man,nstage,nbit_penc,nbit_entry,nadd;
  ...
  if(iflag==0){
    ...
    sprintf((*vd).sde[ie], "  u4: table generic map (IN_WIDTH=>%d,OUT_WIDTH=>%d,TABLE_FILE=>\"ftl%1d.mif\") \n", 
