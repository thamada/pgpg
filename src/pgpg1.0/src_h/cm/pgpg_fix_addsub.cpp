/* --- function list -----------------------------
       void std::generate_pg_fix_addsub(...);
   -----------------------------------------------*/ 
#include<iostream>
#include<cstdio>
#include<cstring>
...
namespace std{
  ...
  void generate_pg_fix_addsub(char sdata[][STRLEN],vhdl_description *vd)
  {
    int nbit,nstage;
    ...
    // +------------------------------+
    strcpy(addsub_flag,sdata[1]);  // ADD or SUB
    strcpy(sx,sdata[2]);           // Name of x-coordinate
    strcpy(sy,sdata[3]);           // Name of y-coordinate
    strcpy(sz,sdata[4]);           // Name of z-coordinate
    nbit = atoi(sdata[5]);         // bit-width parameter (integer)
    nstage = atoi(sdata[6]);       // pipelined parameter (integer)


    // +----------------------------------------------+
    // | Standard output the summary of the arguments |
    // +----------------------------------------------+
    ...
    if(flag==1){
      ...
      sprintf((*vd).sde[ie], "         port map(result=>z,clock=>clk,dataa=>x,datab=>y);         \n" );ie++;
      sprintf((*vd).sde[ie], "                                                                   \n" );ie++;
