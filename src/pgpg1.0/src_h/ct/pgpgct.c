#include<stdio.h>
#include<string.h> // strlen(),strcpy(),strstr(),strcat(),strcmp()
...
struct vhdl_description *vd;
{
  ...
  sprintf(sd[ii], "     Revised by Tsuyoshi Hamada\n" );ii++;
  sprintf(sd[ii], "*/\n" );ii++;
  sprintf(sd[ii], "#include <stdio.h>\n" );ii++;
  sprintf(sd[ii], "#include <math.h>\n" );ii++;
  sprintf(sd[ii], "#ifndef PGINT\n" );ii++;
  sprintf(sd[ii], "#define PGINT long long int\n" );ii++;
  sprintf(sd[ii], "#endif\n" );ii++;
  sprintf(sd[ii], "\n" );ii++;    
...
char *argv[];
{
	...
	while(fgets(isdata,STRLEN,fp)!=NULL){	  
	  ...
	  // --- BITS ---
	  if(strcmp(sdata[0],"pg_bits_or")==0){
	    sprintf(sd.sdpipe[sd.ipipe],"pg_bits_or_%s(%s,%s,&%s);\n",sdata[4],sdata[1],sdata[2],sdata[3]);(sd.ipipe)++;
            sprintf(sd.sddef[sd.idef],"PGINT %s;\n",sdata[1]);(sd.idef)++;
            sprintf(sd.sddef[sd.idef],"PGINT %s;\n",sdata[2]);(sd.idef)++;
	  }
	  if(strcmp(sdata[0],"pg_bits_and")==0){
	    sprintf(sd.sdpipe[sd.ipipe],"pg_bits_and_%s(%s,%s,&%s);\n",sdata[4],sdata[1],sdata[2],sdata[3]);(sd.ipipe)++;
            sprintf(sd.sddef[sd.idef],"PGINT %s;\n",sdata[1]);(sd.idef)++;
            sprintf(sd.sddef[sd.idef],"PGINT %s;\n",sdata[2]);(sd.idef)++;
	  }
	  if(strcmp(sdata[0],"pg_bits_xor")==0){
	    sprintf(sd.sdpipe[sd.ipipe],"pg_bits_xor_%s(%s,%s,&%s);\n",sdata[4],sdata[1],sdata[2],sdata[3]);(sd.ipipe)++;
            sprintf(sd.sddef[sd.idef],"PGINT %s;\n",sdata[1]);(sd.idef)++;
            sprintf(sd.sddef[sd.idef],"PGINT %s;\n",sdata[2]);(sd.idef)++;
	  }
	  if(strcmp(sdata[0],"pg_bits_inv")==0){
	    sprintf(sd.sdpipe[sd.ipipe],"pg_bits_inv_%s(%s,&%s);\n",sdata[3],sdata[1],sdata[2]);(sd.ipipe)++;
            sprintf(sd.sddef[sd.idef],"PGINT %s;\n",sdata[1]);(sd.idef)++;
	  }
	  if(strcmp(sdata[0],"pg_bits_join")==0){
	    sprintf(sd.sdpipe[sd.ipipe],"pg_bits_join_%s_%s(%s,%s,&%s);\n",sdata[4],sdata[5],sdata[1],sdata[2],sdata[3]);(sd.ipipe)++;
            sprintf(sd.sddef[sd.idef],"PGINT %s;\n",sdata[1]);(sd.idef)++;
            sprintf(sd.sddef[sd.idef],"PGINT %s;\n",sdata[2]);(sd.idef)++;
	  }
	  if(strcmp(sdata[0],"pg_bits_part")==0){
	    sprintf(sd.sdpipe[sd.ipipe],"pg_bits_part_%s_%s_%s(%s,&%s);\n",sdata[3],sdata[4],sdata[5],sdata[1],sdata[2]);(sd.ipipe)++;
            sprintf(sd.sddef[sd.idef],"PGINT %s;\n",sdata[1]);(sd.idef)++;
	  }
	  if(strcmp(sdata[0],"pg_bits_rotate")==0){
	    char snrot[20];
	    int snr = atoi(sdata[1]);
	    if(snr<0) sprintf(snrot,"m%d",-snr); else sprintf(snrot,"%d",snr);
	    sprintf(sd.sdpipe[sd.ipipe],"pg_bits_rotate_%s_%s(%s,&%s);\n",sdata[4],snrot,sdata[2],sdata[3]);(sd.ipipe)++;
            sprintf(sd.sddef[sd.idef],"PGINT %s;\n",sdata[2]);(sd.idef)++;
	  }
	  if(strcmp(sdata[0],"pg_bits_delay")==0){
            sprintf(sd.sdpipe[sd.ipipe],"pg_bits_delay_%s_np%s(%s,&%s);\n",sdata[3],sdata[4],sdata[1],sdata[2]);(sd.ipipe)++;
            sprintf(sd.sddef[sd.idef],"PGINT %s;\n",sdata[1]);(sd.idef)++;
	  }
	  ...
	  if(strcmp(sdata[0],"pg_conv_ftol_itp")==0){
            sprintf(sd.sdpipe[sd.ipipe],"pg_conv_ftol_itp_fix%s_log%s_man%s_cut%s(%s,&%s);\n",sdata[3],sdata[4],sdata[5],sdata[7],sdata[1],sdata[2]);(sd.ipipe)++;
            sprintf(sd.sddef[sd.idef],"int %s;\n",sdata[1]);(sd.idef)++;
	  }
	  ...
	  if(strcmp(sdata[0],"pg_conv_ltof_itp")==0){
            sprintf(sd.sdpipe[sd.ipipe],"pg_conv_ltof_itp_log%s_man%s_fix%s_cut%s(%s,&%s);\n",sdata[3],sdata[4],sdata[5],sdata[7],sdata[1],sdata[2]);(sd.ipipe)++;
            sprintf(sd.sddef[sd.idef],"int %s;\n",sdata[1]);(sd.idef)++;
	  }
	  ...
	  if(strcmp(sdata[0],"pg_log_unsigned_add_itp")==0){
            sprintf(sd.sdpipe[sd.ipipe],"pg_log_unsigned_add_itp_log%s_man%s_cut%s(%s,%s,&%s);\n",sdata[4],sdata[5],sdata[7],sdata[1],sdata[2],sdata[3]);(sd.ipipe)++;
            sprintf(sd.sddef[sd.idef],"int %s;\n",sdata[1]);(sd.idef)++;
            sprintf(sd.sddef[sd.idef],"int %s;\n",sdata[2]);(sd.idef)++;	    
	  }
