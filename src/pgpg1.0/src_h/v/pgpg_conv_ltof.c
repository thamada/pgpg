...
struct vhdl_description *vd;
{
  ...
  sprintf((*vd).sde[ie], "  signal sign5 : std_logic;\n" );ie++;
  sprintf((*vd).sde[ie], "  signal sign6 : std_logic;\n" );ie++;
  ...
  sprintf((*vd).sde[ie], "  -------------------------------------------------------------------\n" );ie++;
  sprintf((*vd).sde[ie], "  -- PIPELINE 3,4,5,6,7 STAGES\n" );ie++;
  if(nstage>=3){
    int nbit_exp = nbit_log-nbit_man-2;
  ...
  if(nstage>=1){
    sprintf((*vd).sde[ie], "  process(clk) begin                \n" );ie++;
    sprintf((*vd).sde[ie], "    if(clk'event and clk='1') then  \n" );ie++;
    sprintf((*vd).sde[ie], "      fixdata(%d) <= sign5;         \n",nbit_fix-1 );ie++;
