...
void create_table_dwtable(nbit,nbit_man,nzero,tablecnt)
int nbit,nbit_man,*nzero,tablecnt;
{
  ...
  sprintf(filename,"dwt%1d.mif",tablecnt);
  fp = fopen(filename,"w");
  nentry = 1<<nbit;
  ...
  *nzero = 0;
  for(i=0;i<nentry;i++){
    ...
    if((x>=0.0)&&(x<1.0)){
      ...
      result = -0.75*(2.0-x)*(2.0-x)/x;
    }
    result *= -1;
    iy = 32.0 * log(result * (256.0/3.0))/log(2.0) + 0.5 ;
    if(iy<0){
      iy = 0;
      (*nzero)++;
    }
...
void generate_pg_dwtable(sdata,vd)
char sdata[][STRLEN];
struct vhdl_description *vd;
{
  ...
  ic = (*vd).ic;
  sprintf((*vd).sdc[ic], "  component pg_dwtable_%d_%d                       \n",nbit_log,nbit_man);ic++;
  iflag=0;
  for(j=0;j<(ic-1);j++){
  ...
  im = (*vd).im;
  sprintf((*vd).sdm[im], "  u%d: pg_dwtable_%d_%d ",                    (*vd).ucnt,nbit_log,nbit_man);im++;
  ...
  if(iflag==0){
    ...
    sprintf((*vd).sde[ie], "                                                                   \n" );ie++;   
    sprintf((*vd).sde[ie], "entity pg_dwtable_%d_%d is                        \n",nbit_log,nbit_man );ie++;   
    ...
    sprintf((*vd).sde[ie], "end pg_dwtable_%d_%d;                             \n",nbit_log,nbit_man );ie++;     
    sprintf((*vd).sde[ie], "                                                                   \n" );ie++;   
    sprintf((*vd).sde[ie], "architecture rtl of pg_dwtable_%d_%d is \n",           nbit_log,nbit_man);ie++;   
    sprintf((*vd).sde[ie], "                                                                   \n" );ie++;   
    ...
    create_table_dwtable(nbit_table,nbit_man,&nzero,tablecnt);

    sprintf((*vd).sde[ie], "  u2: unreg_table generic map (IN_WIDTH=>%d,OUT_WIDTH=>%d,TABLE_FILE=>\"dwt%1d.mif\") \n",
