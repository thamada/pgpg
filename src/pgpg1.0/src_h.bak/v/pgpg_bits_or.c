...
void generate_pg_bits_or(sdata,vd)
char sdata[][STRLEN];
struct vhdl_description *vd;
{
  ...
  printf("bits OR : %s = %s | %s : ",sx,sy,sz);
  printf("nbit %d\n",nbit);

  ic = (*vd).ic;
  sprintf((*vd).sdc[ic], "  component pg_bits_or_%d\n",nbit);ic++;
  iflag=0;
  for(j=0;j<ic-1;j++){
  ...
  im = (*vd).im;
  sprintf((*vd).sdm[im], "  u%d: pg_bits_or_%d ",(*vd).ucnt,nbit);im++;
  ...
  sprintf((*vd).sde[ie], "entity pg_bits_or_%d is\n",nbit);ie++;
  ...
  sprintf((*vd).sde[ie], "end pg_bits_or_%d;\n",nbit);ie++;
  sprintf((*vd).sde[ie], "architecture rtl of pg_bits_or_%d is\n",nbit);ie++;
  sprintf((*vd).sde[ie], "begin\n");ie++;
  sprintf((*vd).sde[ie], " z <= x or y;\n");ie++;
