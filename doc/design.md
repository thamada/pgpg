...
対数表現は **符号ビット** | **非ゼロビット** | **指数部** | **仮数部** の構成。底は 2。正の非ゼロ実数 x は y = log₂(x) で表現され、乗算は加算、除算は減算に変換される（GRAPE-5 論文: [Kawai et al. 1999](https://arxiv.org/abs/astro-ph/9909116) 参照）。
...
目標関数: $f_i = \sum_j a_i a_j$。粒子はスカラー a で表現。対数形式（17bit: 符号1, 非ゼロ1, 指数部7, 仮数8）を用い、乗算は加算で実現。出力は固定小数点に変換してアキュムレータで累積。
